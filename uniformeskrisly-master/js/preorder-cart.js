...
(() => {
  ...
  if (btnOpen && cartAPI._hasDrawer()){
    ...
    if (btnCheckout) btnCheckout.addEventListener("click", async () => {
      ...
      const lines = [];
      lines.push("Hola, quiero hacer una PRE-ORDEN de uniformes:");
      lines.push("");

      cart.forEach((it, n) => {
        const talla = it.size ? ` | Talla: ${it.size}` : "";
        const notas = it.notes ? ` | Notas: ${it.notes}` : "";
        const escuela = it.school ? ` (${it.school})` : "";
        lines.push(`${n+1}) ${it.qty} x ${it.name}${escuela}${talla}${notas}`);
      });

      lines.push("");
      lines.push("¿Me apoyan con disponibilidad y cotización, por favor?");
      ...
      const url = "https://wa.me/?text=" + encodeURIComponent(text); // <-- link de WhatsApp
